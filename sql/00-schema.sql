...
CREATE TABLE MONSTRO (
  ...
  CONSTRAINT CK_MONSTRO_EXP_GERADO CHECK (EXP_GERADO >= 0) --Checar se o EXP_GERADO é positivo.
);
...
CREATE TABLE MISSAO (
  ...
  EXP_GERADO NUMERIC NOT NULL,
  TEMPO_FINALIZAR INTERVAL HOUR TO SECOND DEFAULT 60, --Tempo em segundos para finalizar.
...
CREATE TABLE CRIACAO_COMUNIDADE (
  ...
  CONSTRAINT CK_PONTUACAO CHECK (PONTUACAO >= 0) --Checar se a pontuação é positiva.
);
...
CREATE TABLE CONSUMIVEL (
  ITEM VARCHAR(64) NOT NULL,
  TEMPO_DURACAO INTERVAL HOUR TO SECOND NOT NULL, -- SEGUNDOS

  CONSTRAINT PK_CONSUMIVEL PRIMARY KEY(ITEM),
  CONSTRAINT FK_CONSUMIVEL_ITEM FOREIGN KEY(ITEM) 
    REFERENCES ITEM(NOME) ON DELETE CASCADE,

  CONSTRAINT CK_CONSUMIVEL_TEMPO CHECK (TEMPO_DURACAO >= 0) --Checar se o tempo de duração máximo é positivo.
);
...
CREATE TABLE EQUIPAMENTO (
  ...
  CONSTRAINT CK_EQUIPAMENTO_PONTOS_PODER CHECK (PONTOS_PODER >= 0) --Checar se o valor dos pontos de poder extra do equipamento é positivo.
);
...
CREATE TABLE MENSAGEM (
  ...
  CONSTRAINT CK_MENSAGEM_NUMERO_DE_CURTIDAS CHECK ( NUMERO_DE_CURTIDAS >= 0 ) --Checar se o número de curtidadas é positivo.
);
...
CREATE TABLE DOACAO_PARA_COMUNIDADE (
  ...
  CONSTRAINT CK_DOACAO_PARA_COMUNIDADE_VALOR CHECK ( VALOR >= 0 ) --Checar se o valor da doação é positivo.
);
